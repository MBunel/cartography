...
#' x <- sf::st_as_sf(nuts2.spdf)
#' 
#' choroLayer(sf = x, var = "unemprate")
#' 
#' ## Example 2
#' nuts2.df$unemprate <- nuts2.df$unemp2008/nuts2.df$act2008*100
#' choroLayer(sf = x, 
...
#' x$cagr <- (((x$pop2008 / x$pop1999)^(1/9)) - 1) * 100
#' summary(x$cagr)
...
#'             coltitle = "white")
#' @export
choroLayer <- function(sf, spdf, df, spdfid = NULL, dfid = NULL, var, 
                       ...
                       add = FALSE)
{
  ...
  # poly
  plot(x, col = colVec, border = border, lwd = lwd, add = add)
